defmodule WebDriverClient.W3CWireProtocolClientTest do
  ...
  property "fetch_element_rect/1 returns {:ok, %Rect} on valid response", %{
    bypass: bypass,
    config: config
  } do
    check all resp <- TestResponses.fetch_element_rect_response() do
      {config, prefix} = prefix_base_url_for_multiple_runs(config)
      ...
      Bypass.expect_once(
        bypass,
        "GET",
        "/#{prefix}/session/#{session_id}/element/#{element_id}/rect",
        fn conn ->
          conn
          |> put_resp_content_type("application/json")
          |> send_resp(200, resp)
      ...
      assert {:ok, %Rect{x: ^x, y: ^y, width: ^width, height: ^height}} =
               W3CWireProtocolClient.fetch_element_rect(session, element)
    end
  end

  test "fetch_element_rect/2 returns {:error, %UnexpectedResponseFormatErrror on invalid response",
       %{bypass: bypass, config: config} do
    {config, prefix} = prefix_base_url_for_multiple_runs(config)
  ...
  test "fetch_element_rect/2 returns appropriate errors on various server responses", %{
    bypass: bypass,
    config: config
  } do
    scenario_server = set_up_error_scenario_tests(bypass)
